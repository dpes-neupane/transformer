...
class DecoderMultihead(Multihead):
    ...
    def forward(self, x, mask=None, ret_att=False):
        ...
        seq_length //= 3
        qkv = self.qkv(x)
        qkv = qkv.reshape(batch, 3, seq_length, self.heads, self.emb_dim//self.heads) #(batch, qkv, seq_length, heads, dim_k)
        qkv = qkv.permute(0, 1, 3, 2, -1) #(batch, heads, qkv, seq_length, dim_k)
        q, k, v = torch.chunk(qkv, dim=1, chunks=3)
